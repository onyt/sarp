...
function ccToggleCommand(ply, cmd, args)
	...
	if (ply:EntIndex() ~= 0 and not DB.HasPriv(ply, ADMIN)) or (togglecmd.superadmin and not ply:iOwner()) then
...
local function ccTell(ply, cmd, args)
	...
	if ply:EntIndex() ~= 0 and not ply:iAdmin() then
...
local function ccSetMoney(ply, cmd, args)
	if not args[1] then return end
	if ply:EntIndex() ~= 0 and not ply:iOwner() then
